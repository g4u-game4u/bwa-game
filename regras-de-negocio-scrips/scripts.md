...
- **`api-scripts:zoho-cancel-jur-lost-deals`** — `Deals/search` só **Pipeline Jurídico** + **Stage Fechado e Perdido** (sem `Modified_Time`); para cada deal, `POST /game/delivery/{uuid}/cancel` no G4U. `--dry-run`, `--pipeline`, `--stage`, env `G4U_ZOHO_JUR_PIPELINE_LOST_CANCEL` / `G4U_ZOHO_JUR_STAGE_LOST_CANCEL`.
