...
- Montar PAYLOAD para GAME4U (runner `zoho-tasks`)
    - Só deals com **Respons_vel_Jur** ou **Jur_dico_Respons_vel** preenchido; `user_email` = e-mail desse lookup (resolução via API de users do CRM quando necessário).
    - **GET** `Activities_Chronological_View_History`: o runner extrai a lista de atividades, escolhe a **mais recente** cujo JSON contém uma das chaves de **`tagFlowTitleToActionTemplateId`** em `zoho-crm-action-map.json`; o `action_id` é o valor mapeado.
    ...
    - **integration_id estável:** `zoho-deal-{dealId}-jur-action-{actionTemplateId}`. Para **DONE**, o runner obtém `created_at` via **`/user-action/search`** como no fluxo de Stage; sem PENDING correspondente, o DONE é omitido.
    - Opções: `--no-post-process`, `--post-all-mapped`, `--max-deals`, mesmo intervalo `ZOHO_MODIFIED_*` e `pipelineFilter` que `zoho-stages`.
...
5. **`api-scripts:zoho-stages`** — CRM Zoho: **primeira página** do `Deals/search`; para cada deal, `__timeline` (1ª página). Ciclo **DONE** (estágio anterior) + **PENDING** (novo) com **`integration_id`** estável `zoho-deal-{id}-action-{template}`; **`GET /user-action/search`** antes do DONE; **`POST /game/delivery/{id}/complete`** quando o novo Stage está em **`stagesThatCompleteDelivery`**. **`POST /game/action/process`** no formato do front. Por defeito **para no primeiro POST** OK; **`--post-all-mapped`** continua. **`api-scripts:zoho-tasks`** — deals com jurídico: **Activities_Chronological_View_History**, tag → `action_id`, status da task → G4U, **`integration_id`** `zoho-deal-{id}-jur-action-{template}`; mesmas flags úteis (`--no-post-process`, `--post-all-mapped`, `--max-deals`). Mapa: **`src/app/config/zoho-crm-action-map.json`** (front: **`zoho-crm-action-map.ts`**). **`G4U_ZOHO_STAGE_ACTION_ID`** força um único `action_id` em **zoho-stages** (debug)..
...
- `npm run api-scripts:zoho-stages` / `api-scripts:zoho-tasks` — Zoho + logs (env `ZOHO_*`, `ZOHO_MODIFIED_FROM`, `ZOHO_MODIFIED_TO`).
